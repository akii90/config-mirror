...
docker-buildx: ## Build docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name kubebuilderint-builder
	$(CONTAINER_TOOL) buildx use kubebuilderint-builder
	- $(CONTAINER_TOOL) buildx build --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm kubebuilderint-builder
	rm Dockerfile.cross
